...
int main(void) {
    ...
    max7219_write(SCANLIMIT, 3);     /* SCANLIMIT = 3 para 4 dígitos (0-3) */
    max7219_write(INTENSITY, 4);     /* Intensidad 4/16 */
    max7219_write(TEST, 0);          /* Disable test mode */
    max7219_write(SHUTDOWN, 1);      /* Enable device */


    max7219_write(0x01, 5);          /* Dígito 1 = 5 */
    max7219_write(0x02, 7);          /* Dígito 0 = 7 */
    max7219_write(0x03, 3);          /* Dígito 3 = 3 */
    max7219_write(0x04, 8);          /* Dígito 2 = 8 */
